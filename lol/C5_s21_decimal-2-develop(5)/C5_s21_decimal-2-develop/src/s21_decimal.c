...
int s21_add(s21_decimal a, s21_decimal b, s21_decimal *result) {
  ...
  if (a_sign == b_sign) {
    ...
    if (overflow) {
      if (exp > 0) {
        s21_banking_round_for_add(a, b, &result_buffer);
        exp--;
      } else {
    ...
    binary_sub(a, b, &result_buffer);  // TODO: Replace with sub
  }

  if ((a_sign == 1 && b_sign == 1) ||
      (a_sign == 1 ^ b_sign == 1 && s21_compare_two_decimals(a, b) == 2)) {
    set_sign(&result_buffer, 1);

    if (response == 1) {
      response = ERROR_TOO_SMALL;
    }
  }
...
int s21_sub(s21_decimal a, s21_decimal b, s21_decimal *result) {
  ...
  int scale1 = get_pow(a), scale2 = get_pow(b);
...
int s21_div(s21_decimal a, s21_decimal b, s21_decimal *result) {
  ...
  int flag_owerflow_mantisa =
      s21_is_owerflow_mantissa(exp_rez, big_ten_array_result);
