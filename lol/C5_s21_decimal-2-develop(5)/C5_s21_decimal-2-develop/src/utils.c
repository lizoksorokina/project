...
void s21_banking_round_for_add(s21_decimal value_1, s21_decimal value_2,
                               s21_decimal *result) {
  int reminder = 0;
  int is_even = 0;
...
int s21_get_bit_big_decimal(s21_big_decimal value,
                            int position) {  // -1 - ERROR, position 0-127
  int res = -1;
  if ((position > -1 && position < 256)) {
    int byte = position / 32;
...
void s21_from_ten_array_to_big_decimal(int array[], s21_big_decimal *result) {
  ...
  for (int i = 0; i < 96; i++) {
    s21_set_bitt_for_big_decimal(result, i,
                                 s21_get_bit_big_decimal(copy_result, i));
  }
}
...
void div_by_ten(s21_decimal *value) {
  long initial = 0;  // Промежуточное значение при делении
...
int set_pow(s21_decimal value, s21_decimal *result, int int_pow) {
  *result = value;  // Устанавливаем результат равным входному числу
  if (result == NULL) {
    return 1;  // Возвращаем ошибку, если указатель на результат NULL
  }
  if (int_pow <= 28) {
...
void normolize_down(s21_decimal *value, int pow) {
  ...
  set_exp(&temp, 0);
  remainder = binary_div(*value, divider, &result);
...
void bit_substraction(s21_decimal value1, s21_decimal value2,
  ...
  for (int i = 0; i < 96; i++) {
    bit1 = s21_get_bit(value1, i);
    bit2 = s21_get_bit(value2, i);
    sub = bit1 - bit2;
    switch (sub) {
      case (-1): {
        s21_set_bit(res, i, 1);
